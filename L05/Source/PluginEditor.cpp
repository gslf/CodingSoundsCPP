...
void CodingSoundsSynthAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    float gain = audioProcessor.currentGainForDisplay.load();
    ...
    if (note == -1)
        ...
        text << "Last Note: " << juce::MidiMessage::getMidiNoteName(note, true, true, 3)
        << " (" << note << ")";

    text << "\nTotal Note On Events: " << count;

    text << "\n\nGain: " << juce::String(gain, 2);
    text << " (" << juce::String(juce::Decibels::gainToDecibels(gain), 1) << " dB)";
